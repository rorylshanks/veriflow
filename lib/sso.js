...
async function verifyAuth(req, res) {
    try {
        ...
        if (req.session.loggedin) {
            ...
            // If there were any retries, delete the counter after sucesfull login attempt
            if (req.session.auth_retries) {
                req.session.auth_retries = 0
            }
        ...
        let jwtPayload = {
            ...
            vfroute: req.headers["x-veriflow-route-id"]
        }
        ...
        if (await retryAuthFlow(req, res)) return
...
async function setSessionCookie(req, res) {
    ...
    try {
        ...
        if (req.query.token) {
            ...
            let vfroute = decoded.vfroute
            let currentRoute = {}
            if (vfroute) {
                currentRoute = getConfig().policy[vfroute]
            }
            
            redirectProtocol = decoded.protocol
            redirectHost = decoded.host
            redirectPath = decoded.path
            redirectQuery = decoded.query
            ...
            await form_auth.getFormAuthCookie(req, res, currentRoute);
        ...
        if (await retryAuthFlow(req, res, redirect_object)) return
...
async function retryAuthFlow(req, res, redirect_object) {
    try {
        var auth_retries = req.session.auth_retries
        if (!auth_retries || auth_retries <= 3) {
            if (redirect_object) {
                var protocol = redirect_object.redirectProtocol
                var host = redirect_object.redirectHost
                var path = redirect_object.redirectPath
                var query = redirect_object.redirectQuery
            } else {
        ...
        log.error({ message: "Unknown error occoured while retrying previous error...", context: { error: error.message, trace: error.stack } })
        return false
    }
}
...
async function handleRedirectToSetEndpoint(req, res) {
    ...
    let jwtPayload = {
        ...
        parentSession: req.session.id,
        challengeHash: challengeHash,
        vfroute: req.session.redirect.vfroute
    }
    var signedJwt = await createJWT(jwtPayload)
